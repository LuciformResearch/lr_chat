/**
 * Test du moteur de mémoire avancé avec vrais appels LLM
 */

import { AdvancedMemoryEngineLLM } from './advanced-memory-engine-llm';

async function testAdvancedMemoryLLM() {
  console.log('🧪 Test du moteur de mémoire avancé avec vrais appels LLM');
  console.log('============================================================');
  ...
  const engine = new AdvancedMemoryEngineLLM(geminiApiKey, 500, 3, 0.5); // Budget: 500 chars, L1 tous les 3 messages
  ...
  console.log('📝 Simulation d\'une conversation de 16 messages avec vrais appels LLM...');
    ...
    if (action.action !== 'NONE') {
      ...
      if (action.summaries.length > 0) {
        ...
        action.summaries.forEach(summary => {
          console.log(`     - ${summary.id} (L${summary.level}): ${summary.text.slice(0, 100)}...`);
        });
      }
  ...
  // 5. Résumés avec vrais appels LLM
  const hasRealSummaries = actions.some(action => action.summaries.length > 0);
  console.log(`   Résumés LLM générés: ${hasRealSummaries ? '✅' : '❌'}`);
  ...
  console.log(`✅ Résumés LLM générés: ${hasRealSummaries ? 'Oui' : 'Non'}`);
  console.log(`✅ Actions exécutées: ${actions.length}`);
  
  if (budgetOk && compressionActive && contextBuilt && hasRealSummaries) {
    console.log('\n🎉 SUCCÈS: Le moteur de mémoire avancé avec vrais appels LLM fonctionne parfaitement !');
  } else {
...
// Exécuter le test
testAdvancedMemoryLLM().catch(console.error);