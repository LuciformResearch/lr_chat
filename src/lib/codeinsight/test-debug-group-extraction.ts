#!/usr/bin/env node
/**
 * Test de debug - Extraction des scopes individuels
 */
...
async function testDebugGroupExtraction() {
  console.log('🔍 Debug - Extraction des scopes individuels');
  console.log('============================================\n');
        ...
        if (element.children) {
          for (const child of element.children) {
            if (child.type === 'text' && child.content) {
              // Texte simple
              result[child.name || 'text'] = child.content;
              ...
              if (result[child.name]) {
                ...
                result[child.name].push(xmlElementToObject(child));
              } else {
                result[child.name] = xmlElementToObject(child);
              }
            }
          }
        }
        ...
        if (groupAnalysisElement.children) {
          ...
          groupAnalysisElement.children.forEach((child: any, index: number) => {
            ...
            if (child.name === 'scope_analysis') {
              ...
              console.log(`      - children: ${child.children ? child.children.length : 0}`);
              if (child.children) {
                child.children.forEach((grandChild: any, gIndex: number) => {
                  console.log(`        ${gIndex}: type=${grandChild.type}, name=${grandChild.name}, content="${grandChild.content}"`);
                  if (grandChild.name === 'name') {
                    console.log(`          -> Élément name trouvé !`);
                    if (grandChild.children) {
                      grandChild.children.forEach((greatGrandChild: any, ggIndex: number) => {
                        console.log(`            ${ggIndex}: type=${greatGrandChild.type}, content="${greatGrandChild.content}"`);
                      });
                    }
                  }
                });
              ...
              console.log(`    -> Données complètes:`, scopeData);
              if (scopeData.name) {
...
// Exécuter le test
testDebugGroupExtraction().catch(console.error);